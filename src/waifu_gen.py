...
class WaifuGen:
    def __init__(self, arg_default_level="easy"):
        self.src = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "market.conf"
        )
        ...
        with open(self.src, "r") as marketplace:
            ...
            for position in positions:
                try:
                    ...
                    dict_traits[lst_position[0]] = (
                        int(lst_position[1]),
                        lst_position[2].strip(),
                    )
                except:
                    pass
        return dict_traits
    ...
    def _addFetish(
        self, fet_name, fet_attr, used_groups, money, money_spent, lst_fetishes
    ):
    ...
    def roll(self, level=None, money=None):
        ...
        while cnt_failures < 10:
            ...
            if self._ifSkipThisRound(fet_name, fet_attr, used_groups, lst_fetishes):
                continue
            used_groups, money, money_spent, lst_fetishes = self._addFetish(
                fet_name, fet_attr, used_groups, money, money_spent, lst_fetishes
            )
        #
        print("result:")
        print(f"""Level:{level}""")
